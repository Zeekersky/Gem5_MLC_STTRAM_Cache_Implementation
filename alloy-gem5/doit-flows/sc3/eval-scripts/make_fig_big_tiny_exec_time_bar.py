...
parser.add_argument('-o', '--output-file', help="Output plot", default="fig-big-tiny-exec-time-bar.png")
...
for app in app_list:
  for config in config_list_big_tiny:
    if data[app][config_list_big_tiny[0]] > 0:
      data_norm[app][config] = float(data[app][config]) / float(data[app][config_list_big_tiny[0]])
