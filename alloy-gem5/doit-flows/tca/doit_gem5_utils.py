...
def gen_sims_per_app( simdict ):
  ...
  build_dir_path = os.path.abspath(evaldir + '/sim_out/' + basename)
  ...
  for app in app_dict.keys():
    ...
    for group, app_opts_list in app_dict[app].iteritems():
      ...
      for i, app_opts in enumerate(app_opts_list):
        ...
        elif app.split('-')[0] == 'ubmark':
          actions.append( ubmark_verify_cmd(app, app_stdout, summary_file) )
...
def ubmark_verify_cmd( app, app_stdout, summary_file ):
  summarize_cmd = \
      'if grep -q -e \"passed\" ' + app_stdout + '; ' \
      'then echo PASSED : \"'   + app_stdout  + '\" >> ' + summary_file + '; ' \
      'else echo fail\ \  : \"' + app_stdout  + '\" >> ' + summary_file + '; ' \
      'fi'
