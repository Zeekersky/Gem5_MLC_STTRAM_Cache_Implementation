...
class BRG_FUP(FUPool):
    ...
    def init_resources(cls, num_threads):
        ...
        alu_int.count = num_threads
        mem.count     = num_threads
        mdu_int.count = 1
        fpu.count     = 1

        cls.FUList = [alu_int, mdu_int, fpu, mem]

class SMT_IO_CPU(DerivO3CPU):
    @classmethod
    def init_cpu(cls, num_threads,
        ...
        cls.fetchWidth          = num_threads # 1 inst per thread per cycle
        cls.decodeWidth         = num_threads # 1 inst per thread per cycle
        cls.renameWidth         = num_threads # 1 inst per thread per cycle
        cls.dispatchWidth       = num_threads # 1 inst per thread per cycle
        cls.issueWidth          = num_threads # 1 inst per thread per cycle
        cls.wbWidth             = num_threads # 1 inst per thread per cycle
        cls.commitWidth         = num_threads # 1 inst per thread per cycle
        cls.squashWidth         = 16
        ...
        # 8 ROB entries per thread
        cls.numROBEntries           = num_threads * 8
        ...
        #
        cls.LQEntries               = 5 * num_threads
        cls.SQEntries               = 5 * num_threads

        cls.smtNumFetchingThreads   = num_threads

        cls.useMinPCScheme          = use_min_pc
        cls.minPCStallThreshold     = min_pc_threshold

        cls.smtIQPolicy             = "Partitioned"
        cls.smtLSQPolicy            = "Partitioned"
        cls.smtROBPolicy            = "Partitioned"
